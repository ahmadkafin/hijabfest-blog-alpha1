        ...
        <div class="card text-center">
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    ...
                    <li class="nav-item">
<a class="nav-link {{Request::segment(3) == Str::slug($category_single->category_name, '-') ? 'active' : ''}}"
                            href="{{route('categories.show', Str::slug($category_single->category_name, '-'))}}">{{$category_single->category_name}}</a>
                    </li>
                    @endforeach
                </ul>
            </div>
            <div class="text-center">
<h2 class="py-4">List kategori <span
                        style="text-transform: uppercase;">{{Str::of(request()->segment(3))->replace('-', ' ')}}</span>
                </h2>
